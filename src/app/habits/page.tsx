...
export default function HabitsPage() {
  ...
  async function toggleStatus(habitId: number, status: string, habitName: string) {
    ...
    if(res.ok) { alert(`Confirmed: "${habitName}" marked as ${status.toUpperCase()}.`); fetchHabits(); }
  }
    ...
    <div className="max-w-6xl mx-auto space-y-8 p-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => setShowArchived(!showArchived)} className="rounded-2xl border-2 px-6 font-black text-xs uppercase">{showArchived ? "View Active" : "View Archive"}</Button>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button className="rounded-2xl font-black text-xs bg-primary shadow-xl shadow-primary/30 px-8"><Plus className="mr-2 h-4 w-4" /> Create Habit</Button></DialogTrigger>
            <DialogContent className="rounded-[2.5rem] border-4 shadow-2xl p-8">
              ...
              <form onSubmit={createHabit} className="space-y-6 pt-6">
                <div className="space-y-2"><Label className="font-black uppercase text-[10px] tracking-widest">Habit Name</Label><Input className="h-14 rounded-2xl text-xl font-bold" value={name} onChange={(e) => setName(e.target.value)} required /></div>
                <div className="space-y-2"><Label className="font-black uppercase text-[10px] tracking-widest">Description</Label><Input className="h-14 rounded-2xl text-lg" value={description} onChange={(e) => setDescription(e.target.value)} /></div>
                <div className="space-y-2"><Label className="font-black uppercase text-[10px] tracking-widest">Vibe Theme</Label>
        ...
        <AnimatePresence mode="popLayout">
          {habits.map((h, index) => (
            <motion.div key={h.id} layout initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }} transition={{ delay: index * 0.05 }}>
              <Card className={cn("overflow-hidden border-2 rounded-[2.5rem] shadow-xl transition-all duration-300 group hover:-translate-y-2", h.color === 'emerald' ? 'hover:border-emerald-500/50 bg-emerald-500/5' : h.color === 'violet' ? 'hover:border-violet-500/50 bg-violet-500/5' : 'hover:border-rose-500/50 bg-rose-500/5')}>
                <CardHeader className="p-8">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                        <div className={cn("h-16 w-16 rounded-3xl flex items-center justify-center text-3xl font-black text-white shadow-2xl", h.color === 'emerald' ? 'bg-emerald-500 shadow-emerald-500/40' : h.color === 'violet' ? 'bg-violet-500 shadow-violet-500/40' : 'bg-rose-500 shadow-rose-500/40')}>{h.name[0].toUpperCase()}</div>
                        <div>
                    ...
                    <div className="flex flex-col gap-2">
                        <Button variant="ghost" size="icon" onClick={() => toggleArchive(h)} className="hover:bg-muted rounded-full">{h.archived ? <ArchiveRestore className="h-4 w-4" /> : <Archive className="h-4 w-4" />}</Button>
